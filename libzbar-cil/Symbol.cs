...
namespace ZBarSharp
{
    ...
    public class Symbol
    {
        ...
        internal Symbol(IntPtr symbol)
        {
            ...
            //Calculate the bounds of symbol location withim the image, in pixels 
            this.bounds = CalculateBounds(symbol);
        ...
        {
            return this.type.ToString() + " " + this.data + " Bounds: " + this.Bounds.ToString();
        }
        ...
        /// <summary>
        /// Bounding rectangle of the symbol within the containing image. The bounds are given in pixels.
        /// </summary>
        ...
        /// <summary>
        /// Calculates the bounding rect of the decoded symbol
        /// </summary>
        /// <param name="symbol"></param>
        /// <returns></returns>
        private static Rectangle CalculateBounds(IntPtr symbol)
        {
            int minX = int.MaxValue, maxX = int.MinValue, minY = int.MaxValue, maxY = int.MinValue;
            uint locSize = zbar_symbol_get_loc_size(symbol);
            if (locSize == 0)
                return Rectangle.Empty;
            for (uint i = 0; i < locSize; i++)
            {
                //Check whether the x value of the current point is on one of the horizontal bounds
                int val = zbar_symbol_get_loc_x(symbol, i);
                if (val < minX)
                    minX = val;
                if (val > maxX)
                    maxX = val;

                //Check whether the y value of the current point is on one of the vertical bounds
                val = zbar_symbol_get_loc_y(symbol, i);
                if (val < minY)
                    minY = val;
                if (val > maxY)
                    maxY = val;
